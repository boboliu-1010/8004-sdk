...
class Agent:
    ...
    def _extractAgentIdFromReceipt(self, receipt: Dict[str, Any]) -> int:
        ...
        # TRON path: parse Transfer event topics from transaction info.
        tron_logs = receipt.get("log", [])
        if isinstance(tron_logs, list):
            for log in tron_logs:
                try:
                    ...
                    if len(topics) >= 4:
                        ...
                        if topic0 == "ddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef":
                            ...
                            return int(token_hex, 16)
                except Exception:
                    continue

        # TRON fallback: some nodes expose function return in contractResult.
        contract_result = receipt.get("contractResult")
        if isinstance(contract_result, list) and contract_result:
            try:
                return int(str(contract_result[0]), 16)
            except Exception:
                pass
